...
async function addEditCategory(payloadData, userData) {
    try {
        const exit = await Service.findOne(Model.Categories, { name: payloadData.name, _id: { $ne: payloadData.categoryId } })
        if (exit) {
...
async function fetchAllSpecificCategories() {
    try {
        const cond = { parentCategoryId: { $ne: null }, childCategoryId: { $ne: null }, isDeleted: false };
        const data = await Model.Categories.find(cond).populate(["childCategoryId", "parentCategoryId"]);
        return data;
    } catch (error) {
